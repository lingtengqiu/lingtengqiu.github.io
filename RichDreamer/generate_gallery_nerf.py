...
template = """
        <div class="col-md-6 captioned_videos" id="res_{vid_id}" style="margin-top: 0px; margin-bottom: 0px ;">
            <video class="video lazy" loop playsinline autoplay muted style="width: 100%">
                <source data-src="./static/NeRF-Dreamfusion/{keyword}.mp4" type="video/mp4"></source>
            </video>
...
video_tokens = []
keys  = []
with open("./static/nerf-dreamfusion.txt", "r") as f:
    for line in f:
...
for i,k in enumerate(keys):
    ...
    if i - prev_i + 1 >= page_size or last_page:
        ...
        with open(f"nerf_dreamfusion_gallery_{page_idx}.html", "w") as f:
            f.write(html)
        content = ""
        page_idx += 1
        prev_i = i + 1
